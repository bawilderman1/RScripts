...
add_trades_func <- function(data, entry_func, exit_func, strat_cfg) {
  trade_mode <- match.arg(strat_cfg$trade_mode, unlist(TradeDirection))
  
  if (trade_mode == TradeDirection$LONG) {
    data <- data |>
      mutate(
        trd_entry_long = entry_func(data, strat_cfg),
        trd_exit_long = exit_func(data, strat_cfg)
      )
  } else if (trade_mode == TradeDirection$SHORT) {
    data <- data |>
      mutate(
        trd_entry_short = entry_func(data, strat_cfg),
        trd_exit_short = exit_func(data, strat_cfg)
      )
  } else if (trade_mode == TradeDirection$LONG_SHORT) {
    ...
    data <- data |>
      mutate(
        trd_entry_long = strat_cfg$entry_func_long(data, strat_cfg),
        trd_exit_long = strat_cfg$exit_func_long(data, strat_cfg),
        trd_entry_short = strat_cfg$entry_func_short(data, strat_cfg),
        trd_exit_short = strat_cfg$exit_func_short(data, strat_cfg)
      )
  }
...
bt_sim_func <- function(tbl,
  ...
  trade_mode <- strat_cfg$trade_mode
  ...
  pos_state_vec <- character(tbl_rlen)
  ...
  trd_entry_long_vec <- if ("trd_entry_long" %in% names(tbl)) as.logical(tbl$trd_entry_long) else rep(FALSE, tbl_rlen)
  trd_exit_long_vec <- if ("trd_exit_long" %in% names(tbl)) as.logical(tbl$trd_exit_long) else rep(FALSE, tbl_rlen)
  trd_entry_short_vec <- if ("trd_entry_short" %in% names(tbl)) as.logical(tbl$trd_entry_short) else rep(FALSE, tbl_rlen)
  trd_exit_short_vec <- if ("trd_exit_short" %in% names(tbl)) as.logical(tbl$trd_exit_short) else rep(FALSE, tbl_rlen)
  ...
  for (i in seq_len(tbl_rlen)) {
    ...
    entry_long <- trd_entry_long_vec[i]
    exit_long <- trd_exit_long_vec[i]
    entry_short <- trd_entry_short_vec[i]
    exit_short <- trd_exit_short_vec[i]
    
    # --- EXIT --- 
    if (position_state == PositionState$LONG && exit_long) {
    ...
    } else if (position_state == PositionState$SHORT && exit_short) {
      exec_price <- switch(exit_timing,
                           open = open_vec[i],
                           close = close_vec[i],
                           next_open = if (i < tbl_rlen) open_vec[i + 1] else NA_real_
      ...
      if (!is.na(exec_price)) {
        cash_vec[i] <- round(cash_vec[i-1] + (prev_pos_size * exec_price), 2) # Add back the cash from shorting
        pos_size_vec[i] <- 0
        position_state <- PositionState$NONE
      }
    }
    ...
    if (position_state == PositionState$NONE) {
      if (trade_mode != TradeDirection$SHORT && entry_long) {
      ...
      } else if (trade_mode != TradeDirection$LONG && entry_short) {
        exec_price <- switch(entry_timing,
                             open = open_vec[i],
                             close = close_vec[i],
                             next_open = if (i < tbl_rlen) open_vec[i + 1] else NA_real_
    ...
    pos_state_vec[i] <- position_state
    prev_pos_size <- pos_size_vec[i]
    
    # --- Dividends ---
    if (incl_dividends && div_vec[i] > 0) {
      if (pos_size_vec[i] > 0) { # Long position
        pos_size_vec[i] <- pos_size_vec[i] + round((div_vec[i] * pos_size_vec[i]) / open_vec[i], 2)
      } else if (pos_size_vec[i] < 0) { # Short position
        cash_vec[i] <- cash_vec[i] - round(abs(pos_size_vec[i]) * div_vec[i], 2)
      }
    }
  ...
  tbl <- tbl |>
    mutate(
      ...
      pos_state = pos_state_vec,
      drawdn = round(log(eqty / slide_max(eqty, before = Inf)), 4),
      pos_type = case_when(pos_size > 0 ~ "LONG", pos_size < 0 ~ "SHORT", TRUE ~ "FLAT")
    )
    
    if (trade_mode == TradeDirection$LONG) {
        tbl <- tbl |> mutate(trd_num = slide_sum(trd_entry_long, before = Inf))
    } else if (trade_mode == TradeDirection$SHORT) {
        tbl <- tbl |> mutate(trd_num = slide_sum(trd_entry_short, before = Inf))
    } else {
        tbl <- tbl |> mutate(trd_num = slide_sum(trd_entry_long | trd_entry_short, before = Inf))
    }
    
    tbl <- tbl |>
    group_by(trd_num) |>
    mutate(
      is_closed = max(if(exists("trd_exit_long")) trd_exit_long else FALSE | if(exists("trd_exit_short")) trd_exit_short else FALSE),
...
bt_processor <- \(data_tbl,
  ...
  data_val = data_tbl |>
    add_trades_func(entry_func, exit_func, strat_cfg) |>
    bt_sim_func(
      strat_cfg = strat_cfg
    )
