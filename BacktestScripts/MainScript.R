...
### Experimental Setup (Not Working)

# sourceCpp("src/ultimate_smoother.cpp")
# 
# 
# init_eqty <- 10000
# incl_dividends <- TRUE
...
# )
# 
# strtgy_entry <- \(data, cfg) {
#   ifelse(data$rn > 1 & 
#            (data$oc2 > data$UltimateSmoother &
#               lag(data$oc2) <= lag(data$UltimateSmoother)), 
#          1, 0)
# }
# strtgy_exit <- \(data, cfg) {
#   ifelse(data$rn > 1 & 
#            (data$oc2 < data$UltimateSmoother &
#               lag(data$oc2) >= lag(data$UltimateSmoother)), 
#          1, 0)
